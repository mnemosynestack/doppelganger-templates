...
const getCategoryCounts = unstable_cache(
    ...
    ['preset-counts'],
    { tags: ['preset-counts'], revalidate: 3600 }
);
...
export async function getPresets(category?: string, sort?: string, search?: string) {
    ...
    try {
        // Query 1: Get counts for all categories (cached)
    ...
    `;
        ...
        // Process counts
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const totalPresets = countsResult.reduce((sum: number, row: any) => sum + parseInt(row.count), 0);
